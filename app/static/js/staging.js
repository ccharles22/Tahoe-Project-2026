...
let wtStructureScriptPromise = null;

function ensureWtStructureScript() {
  if (window.$3Dmol) return Promise.resolve(window.$3Dmol);
  if (wtStructureScriptPromise) return wtStructureScriptPromise;

  wtStructureScriptPromise = new Promise((resolve, reject) => {
  ...
  return wtStructureScriptPromise;
}

async function initWtStructureViewer() {
  const card = document.querySelector('[data-wt-structure]');
  if (!card) return;

  const canvas = card.querySelector('[data-wt-structure-canvas]');
  const status = card.querySelector('[data-wt-structure-status]');
  const modelUrl = card.getAttribute('data-model-url');
  const accession = card.getAttribute('data-accession') || 'this accession';
  if (!canvas || !status || !modelUrl) return;

  try {
    status.textContent = 'Loading interactive WT structure…';
    const $3Dmol = await ensureWtStructureScript();
    const response = await fetch(modelUrl, { mode: 'cors' });
    if (!response.ok) {
      throw new Error(`Structure file unavailable for ${accession}.`);
    }

    const pdbText = await response.text();
    if (!pdbText || pdbText.trim().length < 40) {
      throw new Error(`No structure model returned for ${accession}.`);
    }

    const viewer = $3Dmol.createViewer(canvas, {
    ...
    status.classList.add('wt-structure__status--error');
    status.textContent = 'Interactive structure preview unavailable here. Use the UniProt or AlphaFold links above.';
  }
}

window.addEventListener('load', initWtStructureViewer);
