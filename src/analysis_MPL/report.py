...
def main():
    ...
    with get_conn() as conn:
        # Stage 4 relies on WT controls as per-generation normalization baselines.
        baselines = fetch_wt_baselines(conn, experiment_id)
        df_variants = fetch_variant_raw(conn, experiment_id)

        # Compute derived metrics and keep a QC dataframe for troubleshooting.
        ...
        # Export ranked summary used by the top-10 table plot and CSV artifact.
        ...
        # Distribution plot uses all derived activity values by generation.
